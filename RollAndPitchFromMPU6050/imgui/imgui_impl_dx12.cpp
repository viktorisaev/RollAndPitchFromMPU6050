...
//	ctx->SetDescriptorHeaps(1, g_hFontSrvGpuDescHandle);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
				...
				ctx->SetGraphicsRootDescriptorTable(1, g_hFontSrvGpuDescHandle);// *(D3D12_GPU_DESCRIPTOR_HANDLE*)&pcmd->TextureId);
...
void ImGui_ImplDX12_NewFrame(ID3D12GraphicsCommandList* cmdList, int w, int h)
{
    if (!g_pPipelineState)
